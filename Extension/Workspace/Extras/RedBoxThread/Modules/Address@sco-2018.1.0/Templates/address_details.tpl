{{#if isNewAddress}}
    ...
    {{#if extraAddressDetails.isMobile}}
...
{{else}}
	...
	{{#if extraAddressDetails.isMobile}}
		...
		{{#if manageOption}}
		<div class="address-details-accordion">
			<div class="address-details-accordion-head">
				...
				<a class="address-details-selector {{#if isSelected}}address-details-selected{{/if}}" data-action="select" data-id="{{internalid}}">
					...
					<span class="addres-details-adress-lines" data-id="{{internalid}}">
						<span class="addres-details-adress-line" data-id="{{internalid}}">{{addressLine1}}</span>
						<span class="addres-details-adress-line" data-id="{{internalid}}">{{addressLine2}}</span>
					</span>
				</a>

				<a class="address-details-accordion-head-toggle {{#unless isSelected}}collapsed{{/unless}}" data-toggle="collapse" data-target="#accordion-{{extraAddressDetails.uniqueid}}" aria-expanded="false" aria-controls="accordion-id">
					<i class="acordion-head-toggle-icon"></i>
				</a>

			</div>
			<div class="address-details-accordion-body collapse{{#if isSelected}} in{{/if}}" id="accordion-{{extraAddressDetails.uniqueid}}">
							...
							<address>
								...
								<div class="address-details-container-multiselect-address-selector">
									<label class="address-details-container-multiselect-address-selector-checkbox">
										<input type="checkbox" {{#if isAddressCheck}} 'checked' {{/if}} data-id="{{internalid}}" data-action="multi-select-address"></input>
									</label>
								</div>
								{{/if}}
