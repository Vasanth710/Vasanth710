<fieldset class="creditcard-edit-form">
    ...
    <p class="creditcard-select-validation-message">Please enter credit card details of card type: 'Visa' or
        'MasterCard'.</p>
    <small class="creditcard-edit-form">{{translate 'Required'}} <span
    ...
    <div class="creditcard-edit-form" data-input="ccnumber" data-validation="control-group">
        ...
        <div class="creditcard-edit-form-controls" data-validation="control">
            <input type="text" class="creditcard-edit-form-input" id="ccnumber" name="ccnumber" value="{{ccnumber}}"
    ...
    <div class="creditcard-edit-form">
        ...
        <div class="creditcard-edit-form-controls-cc-select-container" data-value="creditcard-select-container"
            ...
            <div data-validation="control">
                <!-- Display card type based on card number entered -->
                <p id="display-card-type">Please Select Credit Card Type</p>
                <!-- Hidden input to hold the selected payment method value -->
                <input type="hidden" id="paymentmethod" name="paymentmethod" value="{{paymentMethodValue}}" />
            </div>
        </div>
        {{else}}
        <!-- Hidden input in case showPaymentSelector is false -->
        <input class="creditcard-edit-form-input" type="hidden" id="paymentmethod" name="paymentmethod"
            value="{{paymentMethodValue}}" />
        {{/if}}
        <div class="creditcard-edit-form-controls-img-container" data-value="creditcard-img-container">
            {{#each paymentMethods}}
            <img class="creditcard-edit-form-card-icon {{#if hidden}} hidden {{/if}}" src="{{icon}}"
                data-value="{{key}}" alt="{{name}}" data-image="creditcard-icon" />
            {{/each}}
        </div>
    </div>
    ...
    <div class="creditcard-edit-form">
        <label class="creditcard-edit-form-checkbox">
            <input type="checkbox" id="ccdefault" value="T" data-unchecked-value="F" name="ccdefault" {{#if ccdefault}}
                checked disabled {{/if}}>
            {{translate 'Make this my default credit card'}}
            {{#if ccdefault}} <i class="creditcard-edit-form-info-icon" data-toggle="tooltip"
                title="{{translate 'You need to have at least one card selected by default'}}"></i> {{/if}}
        </label>
    </div>
    {{/if}}
    ...
    <div class="creditcard-edit-form">
        <label class="creditcard-edit-form-checkbox">
            <input type="checkbox" id="savecreditcard" value="T" data-unchecked-value="F" name="savecreditcard" {{#if
                saveCreditCardByDefault}} checked {{/if}}>
