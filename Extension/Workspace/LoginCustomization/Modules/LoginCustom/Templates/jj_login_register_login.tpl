...
<p class="login-register-login-description">
	{{translate 'Login below or <a class="login-register-login-register-now" href="register" data-toggle="show-in-modal"
		data-type="register-now">create an account</a>'}}</p>
{{else}}
<p class="login-register-login-description">
	{{#if extraLoginRegisterLoginView.loginText}}{{{extraLoginRegisterLoginView.loginText}}}{{/if}}</p>
{{/if}}

<small class="login-register-login-required">{{translate 'Required <span
		class="login-register-login-form-required">*</span>'}}</small>
		...
		<div class="login-register-login-form-controls-group" data-validation="control-group">
			...
			<div class="login-register-login-form-controls" data-validation="control">
				<input {{#if hasAutoFocus}} autofocus {{/if}} type="email" name="email" id="login-email"
					class="login-register-login-form-input" placeholder="{{translate 'your@email.com'}}" />
			</div>
		</div>
